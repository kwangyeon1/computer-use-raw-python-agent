...
def _send_request(payload: dict[str, Any], *, timeout_s: float = 5.0) -> dict[str, Any]:
    ...
    while time.monotonic() - started < timeout_s:
        ...
        time.sleep(0.05)
    request_path.unlink(missing_ok=True)
    raise RuntimeError(f"agent daemon did not respond within {timeout_s}s")
...
def start_daemon_process() -> None:
    ...
    subprocess.Popen(
        [sys.executable, "-m", "computer_use_raw_python_agent.daemon", "serve"],
...
def wait_for_daemon_ready(timeout_s: float = 10.0) -> None:
    ...
    while time.monotonic() - started < timeout_s:
        if daemon_is_responding():
            return
        time.sleep(0.1)
    raise RuntimeError(f"agent daemon did not become ready within {timeout_s}s")
...
def main() -> None:
    if len(sys.argv) < 2 or sys.argv[1] != "serve":
        raise SystemExit("usage: python -m computer_use_raw_python_agent.daemon serve")
